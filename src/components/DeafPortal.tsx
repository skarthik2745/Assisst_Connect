  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center mb-6 sm:mb-8">
        ...
        >
          <ArrowLeft size={20} sm:size={24} className="icon-cyan" aria-hidden="true" />
          <span className="text-sm sm:text-base">Back</span>
        </button>
        
        <div className="flex items-center space-x-3 sm:space-x-4">
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center" style={{background: 'rgba(0, 255, 65, 0.2)', border: '2px solid var(--neon-green)'}}>
            <VolumeX size={20} className="icon-green sm:w-6 sm:h-6" aria-hidden="true" />
          </div>
          <div>
            <h1 className="heading-text-green text-xl sm:text-2xl lg:text-3xl">Deaf User Portal</h1>
            <p className="paragraph-text text-sm sm:text-base">Visual communication tools & alerts</p>
          </div>
        </div>
      </div>
      ...
      <section role="region" aria-labelledby="features-grid">
        <h2 id="features-grid" className="heading-text text-lg sm:text-xl lg:text-2xl mb-6 sm:mb-8 text-center">Available Features</h2>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  boxShadow: `0 0 15px ${borderColors[index % 2]}40`
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.boxShadow = `0 0 25px ${borderColors[index % 2]}`
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.boxShadow = `0 0 15px ${borderColors[index % 2]}40`
                }}
                aria-label={`Open ${feature.title}: ${feature.description}`}
              >
                <div className="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 rounded-full flex items-center justify-center mb-3 sm:mb-4 lg:mb-6 mx-auto" style={{background: bgColors[index % 2], border: `2px sm:3px solid ${borderColors[index % 2]}`}}>
                  <IconComponent size={24} className={`${iconColors[index % 2]} sm:w-8 sm:h-8 lg:w-10 lg:h-10`} aria-hidden="true" />
                </div>
